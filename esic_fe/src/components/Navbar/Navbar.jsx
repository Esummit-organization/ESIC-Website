...
export function Navbar() {
	...
	return (
		<nav
			className={`${
				navBackground ? "bg-background-950" : ""
			} duration-300   text-secondary-900 w-screen fixed z-30`}
		>
			<div
				className={`${
					menu ? "text-secondary-900 bg-background-950 w-screen fixed" : ""
				}`}
			>
				<div className="flex place-content-between items-start md:items-center py-4 px-10">
					...
					>
						{options.map((options) => (
							<motion.li
								whileTap={{ scale: 0.9 }}
								className="m-2 cursor-pointer text-white text-xs hover:text-gray-300 duration-150"
							>
								<NavLink
									to={options.to}
									className={({ isActive }) =>
